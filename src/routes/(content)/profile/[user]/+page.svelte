...
<main class="grid w-full gap-8">
	...
	<div class="grid w-full grid-cols-2 gap-2 sm:grid-cols-3">
		{#if $query.data}
			{#each $query.data.pages ?? [] as results (results)}
				{#each results.results as result (result)}
					...
					>
						<h2 class="flex justify-between text-center text-xl font-medium text-snow-300">
							<span class="text-left">#{result.gameNum}</span><span class="text-right"
								>{1 + 6 - result.answers.length / 5} pts</span
							>
						</h2>
