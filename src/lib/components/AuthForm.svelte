...
>
	...
	<slot>
		<button
			class="grid h-full items-center rounded-xl px-6 py-2 text-center font-medium text-snow-100 hover:underline"
			class:text-lg={mode === 'logout'}
			class:text-3xl={mode === 'login'}>{mode === 'login' ? 'Login' : 'Logout'}</button
		>
	</slot>
</form>
